...
- **Monitor RN Firebase releases**: Track [rnfirebase.io](https://rnfirebase.io) and the [CHANGELOG](https://github.com/invertase/react-native-firebase/blob/main/CHANGELOG.md) for SPM support announcement for RN CLI projects.
- **`use_frameworks!` audit**: Other native pods may also require static linking. A full pod audit is needed before removing this flag.
- **v22–v25 sequential migration**: Upgrading from v21.6.1 will require following multiple migration guides. Plan a dedicated sprint slot for this.
- **Google Analytics AdId opt-out**: The current `$RNFirebaseAnalyticsWithoutAdIdSupport = true` flag must be re-applied in the new configuration mechanism (if applicable to the target RN Firebase version).
