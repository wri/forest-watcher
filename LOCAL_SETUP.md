...
# In the project root, install and use the pinned Node version
cd forest-watcher
nvm install    # reads .nvmrc → 14.16.1
nvm use        # activates 14.16.1
```
...
- **Pod install fails:** Make sure you're on the system Ruby or the correct rbenv version. Try `pod repo update` first.
- **Node version mismatch:** Run `nvm use` to switch to 14.16.1 (per `.nvmrc`).
